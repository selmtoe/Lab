...
export function createResearch(ui) {
    ...
    function sourceLink(parent,ref){
        if(!ref)return;
        const match=byId(ref.recordId),video=state.videos.find(v=>v.labMatches.some(m=>m.id===ref.recordId)||(ref.youtubeId&&v.youtubeId===ref.youtubeId));
        parent.append(node('p',`${ref.title||'引用元'} · ${timeText(ref.seconds+(ref.youtubeId?(ref.youtubeOffsetSeconds||0):0))}${ref.player&&ref.player!=='both'?' · '+ref.player.toUpperCase():''}`,'lab-source'));
        if(video)parent.append(button('元動画のこの時刻へ',()=>{state.pendingSource={...ref,youtubeId:video.youtubeId||'',youtubeOffsetSeconds:video.youtubeOffsetSeconds||0};window.router('videos',video.id);}));
    ...
    function capture(data,reference,initial={}){
        ...
        form.append(node('p','元動画の時刻と盤面を一緒に保存します。後から記事への引用・問題化ができます。'));
    ...
    function openDetail(record){
        const d=dialog(record.title);boardPreview(d,record.snapshot,record.sourceRef?.player);sourceLink(d,record.sourceRef);
        ...
        if(record.snapshot)actions.append(button('この局面から練習',()=>openTool('sim',practiceState(record.snapshot),record)),button('テト譜を編集',()=>openTool('editor',record.snapshot,record)));
        if(state.token)actions.append(button('研究メモ・問題を編集',()=>{d.close();editResearch(record);}));
    ...
    function renderWorkspace(id){
        ...
        main.append(node('p','動画やAI採点から集めた局面を整理します。記事を書くときは、編集画面の資料棚から選んで本文へ挿入できます。','lab-help-text'));
        ...
        const render=()=>{list.replaceChildren();const words=search.value.toLowerCase().split(/\s+/).filter(Boolean);
            ...
            for(const record of records.slice(0,visibleCount)){const card=node('article',undefined,'card lab-research-card');card.dataset.recordId=record.id;
                ...
                if(record.research?.ai)body.append(node('p',`AIの候補 · 評価差 ${record.research.ai.gap}点 · ${record.research.ai.player?.toUpperCase()||'P1'}`,'lab-help-text'));
                ...
                if(state.token)actions.append(button('整理・問題化',()=>editResearch(record)));
                body.append(actions);list.append(card);
            }
            if(!records.length)list.append(node('p',sourceMatch?'この試合の採点候補はありません。評価差の基準を満たす手がなかった場合も、採点は完了しています。':'まだ局面がありません。動画の局面表示から「研究ノートに保存」を押すか、AI採点を実行してください。'));
        ...
        if(id&&!sourceMatch){const record=byId(id);if(record)openDetail(record);}
    }
    function scoreReport(report,reference){
        ...
        for(const [index,candidate]of report.candidates.entries()){
            ...
            label.append(check,` 局面 ${candidate.pageIndex+1} · 評価差 ${candidate.gap}点`);item.append(label);boardPreview(item,candidate.snapshot,reference.player);list.append(item);
        }
        ...
        const save=button('選んだ候補を保存',async()=>{if(!selected.size)throw Error('保存する候補にチェックを入れてください。');save.disabled=true;let count=0;
            try{for(const index of [...selected]){const candidate=report.candidates[index],ref=sourceRef(reference);ref.phase=candidate.pageIndex;
                if(Number.isFinite(candidate.time))ref.seconds=(reference._matchStart??reference.startSeconds??0)+candidate.time;
                ...
                await post('/api/records',{kind:'tetofu',title:`${reference.title} · ${ai.player.toUpperCase()} 局面${candidate.pageIndex+1}`.slice(0,250),tags:[...new Set([...(reference.tags||[]),'AI候補'])],
    ...
    function guide(section='workflow'){
        ...
        const groups=section==='editor'?[
            ...
            ['AI採点','動画ページの「AI採点の設定」で対象プレイヤーと考える量を保存します。「この試合をAI採点」または「この動画をまとめてAI採点」で候補の保存まで自動で進みます。エディタ内の採点画面では、範囲を絞った追加調査もできます。']
        ]:[
            ['1. PCの配信解析で解析・修正する','「PCの配信解析を開く」、または「ぷよテト配信を全自動解析.bat」を起動します。今までどおり動画を解析し、盤面・NEXT・HOLD・選手名を修正します。最後に「修正を再判定」を実行して保存してください。Labでは動画解析を実行しません。'],
            ...
            ['3. YouTubeと局面をつなぐ','Videos / Analysis から動画を開きます。「動画・解析結果の設定」でYouTubeのURLを変更できます。時刻がずれる場合は「動画の時刻がずれる場合」で補正します。試合時刻や局面のバーを押すと、埋め込み動画も同じ時刻へ移動します。'],
            ['4. 局面を集める・AIに探させる','気になった時刻でP1 / P2を選び、「研究ノートに保存」します。AI採点はあらかじめ設定を保存し、採点ボタンを押すと気になる手が自動で研究ノートに集まります。'],
            ...
            ['7. Labに公開する','動画設定にYouTubeのURLを貼り、公開したい資料を「公開対象」にします。「公開管理」で内容を見て「Labに公開する」を押します。PCの動画や非公開の研究メモは送信しません。'],
    ...
    return {capture,sourceRef,sourceLink,practiceState,firstPage,boardPreview,editResearch,renderCitations,renderWorkspace,openDetail,scoreReport,guide};
}
